...
class WindowsBluetoothWindowsPlugin : public flutter::Plugin, public flutter::StreamHandler<EncodableValue> {
 public:
  static void RegisterWithRegistrar(flutter::PluginRegistrarWindows *registrar);

  WindowsBluetoothWindowsPlugin();

  virtual ~WindowsBluetoothWindowsPlugin();
 ...
 private:
  ...
  void WindowsBluetoothWindowsPlugin::GattCharacteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args);
};

// static
void WindowsBluetoothWindowsPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows *registrar) {
  auto method =
      std::make_unique<flutter::MethodChannel<EncodableValue>>(
          registrar->messenger(), "windows_bluetooth/method",
          &flutter::StandardMethodCodec::GetInstance());
  auto event_scan_result =
      std::make_unique<flutter::EventChannel<EncodableValue>>(
          registrar->messenger(), "windows_bluetooth/event.scanResult",
          &flutter::StandardMethodCodec::GetInstance());
  auto message_connector_ =
      std::make_unique<flutter::BasicMessageChannel<EncodableValue>>(
          registrar->messenger(), "windows_bluetooth/message.connector",
          &flutter::StandardMessageCodec::GetInstance());

  auto plugin = std::make_unique<WindowsBluetoothWindowsPlugin>();
...
WindowsBluetoothWindowsPlugin::WindowsBluetoothWindowsPlugin() {
  InitializeAsync();
}

WindowsBluetoothWindowsPlugin::~WindowsBluetoothWindowsPlugin() {}

winrt::fire_and_forget WindowsBluetoothWindowsPlugin::InitializeAsync() {
  auto bluetoothAdapter = co_await BluetoothAdapter::GetDefaultAsync();
  bluetoothRadio = co_await bluetoothAdapter.GetRadioAsync();
}

void WindowsBluetoothWindowsPlugin::HandleMethodCall(
    const flutter::MethodCall<flutter::EncodableValue> &method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  ...
  if (method_name.compare("isBluetoothAvailable") == 0) {
    ...
    if (!bluetoothLEWatcher) {
      ...
      bluetoothLEWatcherReceivedToken = bluetoothLEWatcher.Received({ this, &WindowsBluetoothWindowsPlugin::BluetoothLEWatcher_Received });
    }
...
void WindowsBluetoothWindowsPlugin::BluetoothLEWatcher_Received(
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> WindowsBluetoothWindowsPlugin::OnListenInternal(
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> WindowsBluetoothWindowsPlugin::OnCancelInternal(
    const EncodableValue* arguments)
{
  auto args = std::get<EncodableMap>(*arguments);
  auto name = std::get<std::string>(args[EncodableValue("name")]);
...
winrt::fire_and_forget WindowsBluetoothWindowsPlugin::ConnectAsync(uint64_t bluetoothAddress) {
  ...
  auto connnectionStatusChangedToken = device.ConnectionStatusChanged({ this, &WindowsBluetoothWindowsPlugin::BluetoothLEDevice_ConnectionStatusChanged });
...
void WindowsBluetoothWindowsPlugin::BluetoothLEDevice_ConnectionStatusChanged(BluetoothLEDevice sender, IInspectable args) {
...
void WindowsBluetoothWindowsPlugin::CleanConnection(uint64_t bluetoothAddress) {
...
winrt::fire_and_forget WindowsBluetoothWindowsPlugin::RequestMtuAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, uint64_t expectedMtu) {
...
winrt::fire_and_forget WindowsBluetoothWindowsPlugin::SetNotifiableAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::string bleInputProperty) {
  ...
  if (bleInputProperty != "disabled") {
    bluetoothDeviceAgent.valueChangedTokens[characteristic] = gattCharacteristic.ValueChanged({ this, &WindowsBluetoothWindowsPlugin::GattCharacteristic_ValueChanged });
  } else {
    gattCharacteristic.ValueChanged(std::exchange(bluetoothDeviceAgent.valueChangedTokens[characteristic], {}));
  }
}

winrt::fire_and_forget WindowsBluetoothWindowsPlugin::WriteValueAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::vector<uint8_t> value, std::string bleOutputProperty) {
  auto gattCharacteristic = co_await bluetoothDeviceAgent.GetCharacteristicAsync(service, characteristic);
...
void WindowsBluetoothWindowsPluginRegisterWithRegistrar(
  ...
  WindowsBluetoothWindowsPlugin::RegisterWithRegistrar(
