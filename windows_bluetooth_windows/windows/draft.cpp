...
void WindowsBluetoothPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("getPlatformVersion") == 0) {
  ...
  } else if (method_call.method_name().compare("isBluetoothAvailable") == 0){
    ...
    // TODO
    result->Success(nullptr);
  } else if (method_call.method_name().compare("setNotifiable") == 0) {
    auto args = std::get<EncodableMap>(*method_call.arguments());
    auto deviceId = std::get<std::string>(args[EncodableValue("deviceId")]);
    ...
    result->Success(nullptr);
  } else if (method_call.method_name().compare("requestMtu") == 0) {
    auto args = std::get<EncodableMap>(*method_call.arguments());
    auto deviceId = std::get<std::string>(args[EncodableValue("deviceId")]);
    ...
    result->Success(nullptr);
  } else if (method_call.method_name().compare("writeValue") == 0) {
    auto args = std::get<EncodableMap>(*method_call.arguments());
    auto deviceId = std::get<std::string>(args[EncodableValue("deviceId")]);
    auto service = std::get<std::string>(args[EncodableValue("service")]);
    auto characteristic = std::get<std::string>(args[EncodableValue("characteristic")]);
...
void WindowsBluetoothPlugin::BluetoothLEWatcher_Received(
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> WindowsBluetoothPlugin::OnListenInternal(
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> WindowsBluetoothPlugin::OnCancelInternal(
...
winrt::fire_and_forget WindowsBluetoothPlugin::ConnectAsync(uint64_t bluetoothAddress) {
  ...
  auto connnectionStatusChangedToken = device.ConnectionStatusChanged({ this, &WindowsBluetoothPlugin::BluetoothLEDevice_ConnectionStatusChanged });
...
void WindowsBluetoothPlugin::BluetoothLEDevice_ConnectionStatusChanged(BluetoothLEDevice sender, IInspectable args) {
...
void WindowsBluetoothPlugin::CleanConnection(uint64_t bluetoothAddress) {
...
winrt::fire_and_forget WindowsBluetoothPlugin::RequestMtuAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, uint64_t expectedMtu) {
...
winrt::fire_and_forget WindowsBluetoothPlugin::SetNotifiableAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::string bleInputProperty) {
  ...
  if (bleInputProperty != "disabled") {
    bluetoothDeviceAgent.valueChangedTokens[characteristic] = gattCharacteristic.ValueChanged({ this, &WindowsBluetoothPlugin::GattCharacteristic_ValueChanged });
  } else {
    gattCharacteristic.ValueChanged(std::exchange(bluetoothDeviceAgent.valueChangedTokens[characteristic], {}));
  }
}

winrt::fire_and_forget WindowsBluetoothPlugin::WriteValueAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::vector<uint8_t> value, std::string bleOutputProperty) {
  auto gattCharacteristic = co_await bluetoothDeviceAgent.GetCharacteristicAsync(service, characteristic);
...
void WindowsBluetoothPlugin::GattCharacteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args) {
